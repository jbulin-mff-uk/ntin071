...
\begin{frame}{Testing membership in a context-free language}
	...
	Let us have a context-free grammar $G$ \alert{in Chomsky Normal Form}.

	Given an input word $w=a_1\dots a_n\in T^*$, determine if $w\in L(G)$.

	\textbf{Inefficient algorithm:} construct all parse trees from $G$ with 



	\textbf{The Cocke-Younger-Kasami algorithm:} dynamic programming
...
\begin{frame}{The CYK algorithm}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item $X_{ij}$ is the set of variables $A$ such that $A\Rightarrow^* a_ia_{i+1}\ldots a_j$.
		\end{itemize}
	...
	%	$X_{ij}=\{A\rightarrow BC;B\Rightarrow^* a_i\ldots a_k, C\Rightarrow^* a_{k+1}\ldots a_j\}$
