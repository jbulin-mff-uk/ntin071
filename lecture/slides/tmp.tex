    ...
    \begin{definition}[Verifikátor, NP\label{def:NP}]
    \begin{itemize}[<+->]
        \item 
    ...
    \pause
    %\begin{frame}{ Třída $NP$}
    \begin{definition*}[definice \ref{def:NP}]
    \alert{Třída jazyků rozhodnutelných v polynomiálním čase} \alert{$NP$ }  je tvořena jazyky s~polynomiálním verifikátorem.
    \end{definition*}
    \pause
    ...
    \begin{algorithmic}[1]
    \Procedure {CLIQUE\_verifier}{ $\langle\langle G,k\rangle,c\rangle $ }
    %\Repeat\Comment{until convergence}
    %\State {set $\rho_u=0 $ for all actions $u$ }
    \State { Otestuj, jestli $c$ je podgraf $G$ o $k$ uzlech. }
    \State { Otestuj, jestli je $c$ klika, tj. úplný podgraf, má všechny hrany. }
    \State \Return {Oba předchozí testy TRUE?}
    \EndProcedure
    \end{algorithmic}
    \end{algbox}
    \pause\begin{algbox}{Nedeterministický Turingův stroj pro $CLIQUE$}
    \begin{algorithmic}[1]
    \Procedure {CLIQUE\_NTM}{ $\langle G,k\rangle $ }
    %\Repeat\Comment{until convergence}
    %\State {set $\rho_u=0 $ for all actions $u$ }
    \State {Nedeterministicky vyber $k$ prvkovou podmnožinu $c$ vrcholů $G$.}
    \State \Return { Je $c$ klika? tj. úplný podgraf, má všechny hrany.}
    \EndProcedure
    \end{algorithmic}
    \end{algbox}
